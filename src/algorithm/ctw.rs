...
const DEPTH: usize = 56; // context depth in bits (7 bytes)
...
// CTW node-weight prior: weight on a node's own KT estimate vs. its children's
...
pub struct Ctw {
    ...
    lw_est: f64,              // ln(W_EST) — prior weight on a node's own KT estimate
    lw_split: f64,            // ln(1-W_EST) — prior weight on the children's product
    ...
    pub fn update(&mut self, bit: i32) {
        ...
        for d in (0..=DEPTH).rev() {
            ...
            if d == DEPTH {
                ...
                let a = self.lw_est + nd.lpe;
                let b = self.lw_split + child_lw + sib_lw;
