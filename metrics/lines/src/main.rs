//! Prototype: distinct cache-lines touched, via wasm load/store instrumentation.
//!
...
//! meters). Reproducibility caveat: the wasm fuel meter compresses only an 8 KB
//! prefix with minimum tables, so this number reflects that regime, not the
//! full-corpus 5 GB scatter (wasm32's 4 GiB ceiling can't hold the heavy build).
...
fn run(wasm: &[u8]) {
    ...
    let a = count(8192);
    let b = count(8192);
    assert_eq!(a, b, "non-deterministic line count");
    println!(
        "LINES: {} distinct 64B lines ({} bytes touched; deterministic, counts heap+static+stack)",
        a,
        a as u64 * LINE
    );
}
